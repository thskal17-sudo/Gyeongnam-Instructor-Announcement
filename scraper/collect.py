...
EXCLUDE = re.compile(r"(수강생|교육생|학습자|참가자|장학생|수료식)\s*(모집|안내)")
...
def merge(existing: list[dict], new: list[dict]) -> tuple[list[dict], int]:
    ...
    by_key = {(a["institution_id"], a["title"]): a for a in existing}
    added = 0
    for item in new:
        key = (item["institution_id"], item["title"])
