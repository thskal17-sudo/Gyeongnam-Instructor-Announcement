...
| `CERTIFICATE_VERIFY_FAILED` | 서버가 중간 인증서를 안 보냄 (인제대 본교) | 브라우저는 열리지만 httpx 는 실패. 소스별 검증 완화 옵션이 gia 에 없으므로 비활성 + notes |
| `SSLV3_ALERT_HANDSHAKE_FAILURE` | TLS 1.0/약한 암호만 지원하는 구형 서버 (창신대·창원문성대·거제대 평생교육원) | `scripts/dump_structure.py` 는 SECLEVEL 낮춘 컨텍스트로 재시도해 구조는 볼 수 있음. gia 수집은 HTTP 클라이언트에 같은 폴백이 들어와야 가능 |
